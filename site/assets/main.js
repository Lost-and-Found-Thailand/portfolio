(function () {
  ...
  /* ----------------------------------------------------------------
     Ambient shader wash behind the hero network diagram. One WebGL
     canvas, zero dependencies — adapted from the 21st.dev "Neuro
     Noise" preset (itself adapted from Paper Shaders, Apache-2.0),
     recoloured to the brand's black/lime palette and toned down (low
     intensity, a vignette that fades to the page's own black) so it
     reads as a slow-moving glow behind the network diagram rather
     than competing with the hero copy. Skipped under reduced motion,
     same as every other animated effect on this page.
     ---------------------------------------------------------------- */
  if (!reduceMotion) {
    var shaderCanvases = document.querySelectorAll("[data-shader-bg]");
    shaderCanvases.forEach(function (canvas) { initShaderBackground(canvas); });
  }
  ...
  function initShaderBackground(canvas) {
    ...
    var FRAG = "#ifdef GL_FRAGMENT_PRECISION_HIGH\n" +
      ...
      "  glow = smoothstep(0.02, 0.13, glow);\n" +
    ...
    /* Palette: brand black rising through a muted olive-lime, never
       reaching the full-brightness --color-accent (#C8FF00) -- this
       runs behind body copy, so it stays dark and low-contrast on
       purpose. OKLab mixing (u_oklab) keeps the transitions smooth
       rather than muddy in between stops. */
    var UNIFORMS = {
      colors: [
        [0.00784, 0.00784, 0.00392],
        [0.03922, 0.06275, 0.01569],
        [0.09412, 0.15686, 0.03137],
        [0.18824, 0.29020, 0.03922],
        [0.35294, 0.50980, 0.05882],
        [0.35294, 0.50980, 0.05882],
        [0.35294, 0.50980, 0.05882],
        [0.35294, 0.50980, 0.05882]
      ],
    ...
    if (!gl.getProgramParameter(program, gl.LINK_STATUS)) {
      ...
      console.error("vertex compile:", gl.getShaderInfoLog(vertexShader));
      console.error("fragment compile:", gl.getShaderInfoLog(fragmentShader));
    }
    ...
    var resizeCanvas = function () {
      ...
      var pixelScale = Math.min(1, Math.sqrt(250000 / Math.max(1, rawWidth * rawHeight)));
    ...
    var updatePointerTarget = function () {
